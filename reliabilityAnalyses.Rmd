  ...
  for(d in 1:length(days)){
    ...
    for(variable in 4:ncol(temp)){ #Calculate the mean ICC for each outcome
      ...
      for(rep in 1:ntimes){
        ...
        # If there are differences between week and weekend days
        if(wkwkd_differences$p_value[which(wkwkd_differences$hours == hours[h] & wkwkd_differences$outcome ==
                                           colnames(temp)[variable])] < .05){ 
            sample <- temp %>% group_by(id, weekday) %>% slice_sample(n = days[d]-1) # Include at least one weekend day
        } else{
            sample <- temp %>% group_by(id) %>% slice_sample(n = days[d])
        }
...
write.csv(ICC.wrist.ENMO, file = paste0(savedir, "Axivity/wrist/ICC_Spearman_MAD.csv"))
```
