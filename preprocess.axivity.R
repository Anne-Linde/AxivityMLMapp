#' preprocess.axivity
#'
#' @description 'preprocess.axivity' Preprocesses the epoch level data by removing non-wear, and invalid data
#'
...
preprocess.axivity <- function(epochdir, epochlength = 5, minhours = 8, mindays = 7, savedir, castordata){
    ...
    if(length(data_day) > 1){
      ...
      for (day in 1:length(data_day)) {
        ...
        if(length(index_nw) > 1){
          data_day[[day]] <- data_day[[day]][-index_nw,] # Remove non-wear data
        }
